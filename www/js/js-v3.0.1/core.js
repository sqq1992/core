        ...
        $.ajax({
            ...
            beforeSend: function (jqXHR) {
                ...
                if (num != -1) {
                    ...
                    console.log('Biu~Abort掉一个请求：' + udt.api);
                    jqXHR.abort();
                } else {
...
(function (app, win) {
    ...
    function execComplete(modName) {
        ...
        //console.log(modMap[modName].deps, modMap[modName], modName);
    ...
    function use(deps, callback, option) {
        ...
        var params = [];
        console.log('======[' + deps + '] is onloading');
        for (var i = 0; i < deps.length; i++) {
            (function (j) {
                loadMod(deps[j], function (param) {
                    console.log('**' + deps[j] + ' loaded');
                    depsCount--;
                    params[j] = param;
                    if (depsCount === 0) {
                        callback.apply(null, params);
                        console.log('@@' + option.baseUrl + '  finished');
                    }
                }, option);
            }(i));
        }
